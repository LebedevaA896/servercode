...
// add складывает два числа в поле GF(2^8). Это также работает как вычитание,
// потому что операция симметрична.
...
func Split(secret []byte, parts, threshold int) ([][]byte, error) {
	...
	if parts < threshold {
		return nil, fmt.Errorf("частей не может быть меньше порога")
	}
	if threshold < 2 {
		return nil, fmt.Errorf("порог должен быть не менее 2")
	}
	if len(secret) == 0 {
		return nil, fmt.Errorf("нельзя разбить пустой секрет")
	}
	...
	for idx, val := range secret {
		...
		if err != nil {
			return nil, fmt.Errorf("не удалось сгенерировать полином: %w", err)
		}
	...
	return out, nil 	// Возвращение разделенного секрета
}

// Combine восстанавливает секрет из частей, если число частей больше или равно порогу.
func Combine(parts [][]byte) ([]byte, error) {
	if len(parts) < 2 { // Проверка, что частей достаточно для восстановления секрета
		return nil, fmt.Errorf("менее 2 частей не могут быть использованы для восстановления секрета")
	}
	firstPartLen := len(parts[0])
	if firstPartLen < 2 {
		return nil, fmt.Errorf("части должны быть не менее двух байтов")
	}
	for i := 1; i < len(parts); i++ {
		if len(parts[i]) != firstPartLen {
			return nil, fmt.Errorf("все детали должны быть одинаковой длины")
		}
	}
	secret := make([]byte, firstPartLen-1) // Буфер для восстановленного секрета
	...
	for i, part := range parts {
		...
		if exists := checkMap[samp]; exists {
			return nil, fmt.Errorf("обнаружен дубликат детали")
		}
		checkMap[samp] = true
		x_samples[i] = samp
	}
	for idx := range secret { // Восстановление каждого байта секрета
		for i, part := range parts { 	// Сбор точек (x, y) для интерполяции
			y_samples[i] = part[idx]
		}
		// Интерполяция полинома и вычисление значения для x = 0
		val := interpolatePolynomial(x_samples, y_samples, 0)
		secret[idx] = val	// Восстановление значения секрета для текущего байта
	}
	return secret, nil
}
